...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrInvalidTransition), errors.Is(err, domain.ErrNotReady):
		return "invalid_transition", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrValidationBlocked), errors.Is(err, domain.ErrInvariant):
