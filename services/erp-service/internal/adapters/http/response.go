...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrPeriodClosed):
		return "period_closed", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrUnbalanced):
		return "journal_unbalanced", http.StatusUnprocessableEntity, false, message
	case errors.Is(err, domain.ErrApprovalRequired):
		return "approval_required", http.StatusForbidden, false, message
