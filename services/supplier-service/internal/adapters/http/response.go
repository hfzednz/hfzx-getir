...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrConflict), errors.Is(err, domain.ErrAlreadyExists), errors.Is(err, domain.ErrIdempotencyConflict):
		return "conflict", 409, false, message
	case errors.Is(err, domain.ErrForbidden), errors.Is(err, domain.ErrUnauthorized), errors.Is(err, domain.ErrApprovalRequired):
		return "forbidden", 403, false, message
	case errors.Is(err, domain.ErrNotVerified):
		return "supplier_not_verified", 403, false, message
