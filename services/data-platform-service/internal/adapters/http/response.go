...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrSchemaIncompatible):
		return "schema_incompatible", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrQualityFailed):
		return "quality_failed", http.StatusUnprocessableEntity, false, message
	default:
