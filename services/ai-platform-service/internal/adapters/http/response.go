...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrModelNotReady):
		return "model_not_ready", http.StatusServiceUnavailable, true, message
	case errors.Is(err, domain.ErrGuardrailBlocked):
		return "guardrail_blocked", http.StatusUnprocessableEntity, false, message
	case errors.Is(err, domain.ErrProviderUnavailable):
		return "provider_unavailable", http.StatusBadGateway, true, message
	default:
