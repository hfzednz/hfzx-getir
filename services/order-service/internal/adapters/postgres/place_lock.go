...
// AdvisoryPlaceLocker uses Postgres session advisory locks for PlaceOrder idempotency.
type AdvisoryPlaceLocker struct{ DB *sql.DB }

func (l *AdvisoryPlaceLocker) WithLock(ctx context.Context, key string, fn func() error) error {
	if l == nil || l.DB == nil {
		return fn()
	}
