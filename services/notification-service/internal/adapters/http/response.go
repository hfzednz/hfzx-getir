...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrSuppressed):
		return "suppressed", http.StatusOK, false, message
	case errors.Is(err, domain.ErrProviderFailed), errors.Is(err, domain.ErrMaxRetries):
		return "provider_failed", http.StatusBadGateway, true, message
