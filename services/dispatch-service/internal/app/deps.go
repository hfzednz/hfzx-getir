...
// Deps aggregates application ports for dispatch use cases.
type Deps struct {
	Dispatches ports.DispatchRepo
	Couriers   ports.CourierPool
	Vehicles   ports.VehicleRepo
...
func (d *Deps) recordEvent(ctx context.Context, disp domain.Dispatch, eventType string, from, to domain.DispatchStatus, extra map[string]any) {
	if d.Dispatches == nil {
		return
	}
	_ = d.Dispatches.AppendEvent(ctx, domain.DispatchEvent{
		ID: d.newID(), TenantID: disp.TenantID, DispatchID: disp.ID,
		Type: eventType, FromStatus: from, ToStatus: to,
		Payload: extra, CreatedAt: d.now(),
	})
}
