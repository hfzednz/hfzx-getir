...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrRateLimited):
		return "rate_limited", 429, true, message
	case errors.Is(err, domain.ErrForbidden), errors.Is(err, domain.ErrUnauthorized), errors.Is(err, domain.ErrPermissionDenied):
		return "forbidden", 403, false, message
	case errors.Is(err, domain.ErrIllegalTransition), errors.Is(err, domain.ErrIncompatible):
		return "illegal_transition", 409, false, message
	case errors.Is(err, domain.ErrNotSigned), errors.Is(err, domain.ErrSandboxViolation):
		return "sandbox_violation", 403, false, message
	default:
