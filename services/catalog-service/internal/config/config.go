...
// Config is the runtime configuration for catalog-service.
type Config struct {
	HTTPAddr     string
	GRPCAddr     string
...
func Load() (Config, error) {
	cfg := Config{
		...
		PublicBaseURL:      strings.TrimRight(env("PUBLIC_BASE_URL", "http://localhost:8082"), "/"),
		DatabaseURL:        env("DATABASE_URL", ""),
		RedisURL:           env("REDIS_URL", ""),
		KafkaBrokers:       splitCSV(env("KAFKA_BROKERS", "")),
		OpenSearchURL:      env("OPENSEARCH_URL", ""),
		MediaServiceURL:    env("MEDIA_SERVICE_URL", "http://localhost:8085"),
...
// EnvDuration helper for future TTL config.
