...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrInvalidArgument), errors.Is(err, domain.ErrInvalidSlug), errors.Is(err, domain.ErrInvalidBarcode):
	...
	case errors.Is(err, domain.ErrInvariant), errors.Is(err, domain.ErrCategoryCycle):
		return "invariant_violation", http.StatusUnprocessableEntity, false, message
	case errors.Is(err, domain.ErrProductDeleted), errors.Is(err, domain.ErrProductNotEditable):
		return "product_state", http.StatusConflict, false, message
	default:
