...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	message = err.Error()
	switch {
	case errors.Is(err, domain.ErrNotFound), errors.Is(err, domain.ErrMissingTranslation):
	...
	case errors.Is(err, domain.ErrCountryInactive):
		return "country_inactive", 409, false, message
