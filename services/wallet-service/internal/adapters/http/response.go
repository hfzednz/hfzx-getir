...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	message = err.Error()
	switch {
	case errors.Is(err, domain.ErrNotFound), errors.Is(err, domain.ErrHoldNotFound):
	...
	case errors.Is(err, domain.ErrOverdraft), errors.Is(err, domain.ErrLimitExceeded),
		errors.Is(err, domain.ErrInvariant), errors.Is(err, domain.ErrHoldReleased):
