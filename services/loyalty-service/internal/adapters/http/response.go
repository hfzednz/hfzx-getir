...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrInvalidArgument), errors.Is(err, domain.ErrReferralInvalid):
		return "invalid_argument", http.StatusBadRequest, false, message
	case errors.Is(err, domain.ErrSelfReferral), errors.Is(err, domain.ErrForbidden):
	...
	case errors.Is(err, domain.ErrConflict), errors.Is(err, domain.ErrIdempotencyConflict),
		errors.Is(err, domain.ErrCashbackState):
		return "conflict", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrInsufficientPoints), errors.Is(err, domain.ErrInvariant):
