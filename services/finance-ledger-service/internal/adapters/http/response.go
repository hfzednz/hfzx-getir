...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrUnbalancedJournal), errors.Is(err, domain.ErrInvariant):
		return "invariant_violation", http.StatusUnprocessableEntity, false, message
	case errors.Is(err, domain.ErrJournalImmutable), errors.Is(err, domain.ErrInvoiceImmutable),
