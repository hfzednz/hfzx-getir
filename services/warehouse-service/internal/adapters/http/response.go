...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrConflict), errors.Is(err, domain.ErrTaskNotClaimable):
		return "conflict", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrInvalidTransition),
		errors.Is(err, domain.ErrRemainingQty),
		errors.Is(err, domain.ErrWeightMismatch):
		return "invalid_transition", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrBarcodeMismatch):
		return "barcode_mismatch", http.StatusUnprocessableEntity, false, message
