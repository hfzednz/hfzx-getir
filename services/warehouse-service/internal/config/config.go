// Package config loads warehouse-service settings from the environment.
package config
...
// Config is the runtime configuration for warehouse-service.
type Config struct {
	HTTPAddr             string
	GRPCAddr             string
...
func Load() (Config, error) {
	cfg := Config{
		...
		PublicBaseURL:       strings.TrimRight(env("PUBLIC_BASE_URL", "http://localhost:8088"), "/"),
		...
		WeightToleranceG:    int64(envInt("WEIGHT_TOLERANCE_G", 50)),
	}
	if cfg.HTTPAddr == "" {
