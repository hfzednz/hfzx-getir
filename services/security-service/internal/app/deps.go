...
type Deps struct {
	...
	Secrets    ports.SecretRepo
	Threats    ports.ThreatRepo
	Vulns      ports.VulnRepo
	Incidents  ports.IncidentRepo
	Compliance ports.ComplianceRepo
	DataGov    ports.DataGovRepo
	Risks      ports.RiskRepo
	Access     ports.AccessRepo
	Devices    ports.DeviceRepo
	AISec      ports.AISecRepo
	FraudSigs  ports.FraudRepo
	Outbox     ports.OutboxRepository
	Publisher  ports.EventPublisher
	Vault      ports.VaultClient
	OPA        ports.OPAClient
	Identity   ports.IdentityTrustClient
	Fraud      ports.FraudFacadeClient
	SIEM       ports.SIEMClient
	SOAR       ports.SOARClient
	AIGuard    ports.AIGuardrailClient
	Clock      ports.Clock
	IDs        ports.IDGen
}
...
func (d *Deps) emit(ctx context.Context, tenantID, aggregateID uuid.UUID, eventType string, extra map[string]any) {
	...
	if d.SIEM != nil && (eventType == domain.EventSecurityAlertCreated || eventType == domain.EventThreatDetected) {
		_ = d.SIEM.SendAlert(ctx, tenantID, payload)
	}
}
