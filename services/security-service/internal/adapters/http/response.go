...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrPolicyDenied):
		return "policy_denied", http.StatusForbidden, false, message
	...
	case errors.Is(err, domain.ErrSecretNotRotatable):
		return "secret_not_rotatable", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrComplianceFailed):
		return "compliance_failed", http.StatusUnprocessableEntity, false, message
	case errors.Is(err, domain.ErrIllegalTransition):
		return "illegal_transition", http.StatusConflict, false, message
