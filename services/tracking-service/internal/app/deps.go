...
// Deps aggregates application ports for tracking use cases.
type Deps struct {
	Locations     ports.LocationRepo
	Timelines     ports.TimelineRepo
...
func (d *Deps) historyCap() int {
	if d.HistoryCap > 0 {
		return d.HistoryCap
	}
	return domain.DefaultHistoryCap
}

func (d *Deps) arrivalThreshold() float64 {
	if d.ArrivalThresh > 0 {
		return d.ArrivalThresh
	}
	return domain.DefaultArrivalThresholdM
}
