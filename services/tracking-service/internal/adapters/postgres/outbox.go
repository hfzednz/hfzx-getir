...
func (r *OutboxRepo) Enqueue(ctx context.Context, msg domain.OutboxMessage) error {
	_, err := r.DB.ExecContext(ctx, `
		...
		string(msg.Status), msg.Attempts, msg.LastError, msg.CreatedAt.UTC(), msg.UpdatedAt.UTC(),
...
func (r *OutboxRepo) Update(ctx context.Context, msg domain.OutboxMessage) error {
	res, err := r.DB.ExecContext(ctx, `
		...
		WHERE id=$1`,
		msg.ID, msg.Topic, msg.Key, JSONMap(msg.Payload), string(msg.Status), msg.Attempts, msg.LastError,
...
func (r *OutboxRepo) ListPending(ctx context.Context, limit int) ([]domain.OutboxMessage, error) {
	...
	rows, err := r.DB.QueryContext(ctx, `
		SELECT id, tenant_id, aggregate_id, topic, key, payload, status, attempts, last_error,
			created_at, updated_at, published_at
		FROM outbox WHERE status=$1 ORDER BY created_at ASC LIMIT $2`,
		string(domain.OutboxStatusPending), limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
