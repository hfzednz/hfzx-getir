...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	case errors.Is(err, domain.ErrInvalidArgument), errors.Is(err, domain.ErrNegativeQuantity):
	...
	case errors.Is(err, domain.ErrInvalidTransition),
		...
		errors.Is(err, domain.ErrReservationInactive):
		return "invalid_transition", http.StatusConflict, false, message
	case errors.Is(err, domain.ErrInsufficientStock), errors.Is(err, domain.ErrLotNotAllocatable):
		return "insufficient_stock", http.StatusConflict, false, message
