...
func (r *OutboxRepo) Enqueue(ctx context.Context, msg domain.OutboxMessage) error {
	_, err := r.DB.ExecContext(ctx, `
		...
		msg.Status, msg.Attempts, msg.LastError, msg.CreatedAt.UTC(), msg.UpdatedAt.UTC(), nullTime(msg.PublishedAt))
	return err
}

func (r *OutboxRepo) Update(ctx context.Context, msg domain.OutboxMessage) error {
	res, err := r.DB.ExecContext(ctx, `
		UPDATE open_outbox SET topic=$2, key=$3, payload=$4, status=$5, attempts=$6, last_error=$7,
			updated_at=$8, published_at=$9 WHERE id=$1`,
...
func (r *OutboxRepo) ListPending(ctx context.Context, limit int) ([]domain.OutboxMessage, error) {
	...
	rows, err := r.DB.QueryContext(ctx, `
		SELECT id, tenant_id, aggregate_id, topic, key, payload, status, attempts, last_error,
			created_at, updated_at, published_at
		FROM open_outbox WHERE status=$1 ORDER BY created_at ASC LIMIT $2`, domain.OutboxStatusPending, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.TenantID, &m.AggregateID, &m.Topic, &m.Key, &payload, &m.Status, &m.Attempts, &m.LastError,
