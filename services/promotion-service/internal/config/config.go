// Package config loads promotion-service settings from the environment.
package config
...
// Config is the runtime configuration for promotion-service.
type Config struct {
	HTTPAddr           string
	GRPCAddr           string
...
func Load() (Config, error) {
	cfg := Config{
		...
		PublicBaseURL:      strings.TrimRight(env("PUBLIC_BASE_URL", "http://localhost:8094"), "/"),
