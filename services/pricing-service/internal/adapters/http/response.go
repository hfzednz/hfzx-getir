...
func mapError(err error) (code string, status int, retriable bool, message string) {
	...
	message = err.Error()
	switch {
	case errors.Is(err, domain.ErrNotFound), errors.Is(err, domain.ErrPriceNotFound):
	...
	case errors.Is(err, domain.ErrInvalidArgument), errors.Is(err, domain.ErrNegativeMoney),
		errors.Is(err, domain.ErrCurrencyMismatch), errors.Is(err, domain.ErrEmptyQuote):
	...
	case errors.Is(err, domain.ErrInvariant), errors.Is(err, domain.ErrInvalidTransition):
