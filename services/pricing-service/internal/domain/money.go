...
// ApplyBps applies basis-points adjustment (can increase). Result must stay non-negative.
func (m Money) ApplyBps(bps int) (Money, error) {
	if err := m.Validate(); err != nil {
		return Money{}, err
	}
	delta := m.AmountMinor * int64(bps) / 10000
	out := m.AmountMinor + delta
	if out < 0 {
		return Money{}, fmt.Errorf("%w: negative after bps", ErrNegativeMoney)
	}
	return Money{AmountMinor: out, Currency: m.Currency}, nil
}
