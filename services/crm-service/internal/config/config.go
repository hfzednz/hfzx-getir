...
// Config is the runtime configuration for crm-service.
type Config struct {
	HTTPAddr           string
	GRPCAddr           string
...
func Load() (Config, error) {
	cfg := Config{
		HTTPAddr:           env("HTTP_ADDR", ":8102"),
		GRPCAddr:           env("GRPC_ADDR", ":9102"),
		PublicBaseURL:      strings.TrimRight(env("PUBLIC_BASE_URL", "http://localhost:8102"), "/"),
