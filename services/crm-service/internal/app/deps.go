...
// Deps aggregates application ports for CRM use cases.
type Deps struct {
	Tickets      ports.TicketRepo
	Chats        ports.ChatRepo
	Agents       ports.AgentRepo
	KB           ports.KBRepo
	Cases        ports.CaseRepo
	Feedback     ports.FeedbackRepo
	SLA          ports.SLARepo
	Outbox       ports.OutboxRepository
	Profiles     ports.ProfileReadClient
	Orders       ports.OrderReadClient
	Notify       ports.NotificationClient
	Refunds      ports.RefundRequestClient
	LLM          ports.LLMClient
	Publisher    ports.EventPublisher
	Clock        ports.Clock
	IDs          ports.IDGen
	AIConfidence float64 // escalate below this (default 0.5)
}
...
func (d *Deps) confidenceThreshold() float64 {
	if d.AIConfidence > 0 {
		return d.AIConfidence
	}
	return 0.5
}
