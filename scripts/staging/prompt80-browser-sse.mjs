...
function warehousePick() {
  ...
  const out = execSync(
    `curl -sS -o /tmp/p80-pick.json -w "%{http_code}" -X POST -H "Content-Type: application/json" -H "X-Tenant-Id: ${TENANT}" -H "Authorization: Bearer ${tok}" -d "{}" http://127.0.0.1:8113/v1/warehouse/tasks/${ORDER}/pick`,
    { encoding: "utf8" },
  );
  console.log("warehouse_pick_http", out.trim());
  console.log("warehouse_pick_body", fs.readFileSync("/tmp/p80-pick.json", "utf8").slice(0, 240));
}
...
(async () => {
  ...
  await page.goto(BASE + "/search?q=milk", { waitUntil: "domcontentloaded", timeout: 30000 });
  console.log("search_ok", page.url());
  console.log("mobile_overflow_search", await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth));
  ...
  console.log("mobile_overflow_track", await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth));
  ...
  const statusEl = page.getByTestId("track-status");
  if (await statusEl.count()) {
    console.log("track_status_ui", ((await statusEl.textContent()) || "").trim());
  }

  await page.goto(BASE + "/cart", { waitUntil: "domcontentloaded", timeout: 20000 });
  console.log("cart_ok");
  console.log("mobile_overflow_cart", await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth));
  ...
  console.error("PROMPT80_BROWSER_FAIL", err);
  process.exit(1);
});
